...
data_write_path = 'raw/domiporta_rental_listings.csv'
...
listing_category_urls = {'mieszkanie/wynajme': 'apartments', 'dom/wynajme': 'houses', 'pokoj/wynajme': 'rooms'}

# Loop through the listing pages
for category_url, category_name in listing_category_urls.items():
    page_number = 1 # This is the start pagek
    while True:
        print(f'\nSearching listings in page {page_number}... {root_url}/{category_url}?PageNumber={page_number}')
        data = pd.DataFrame()
        listings_page = requests.get(f'{root_url}/{category_url}?PageNumber={page_number}')
        ...
        for single_listing in listings:
            ...
            try:
                ...
                single_listing_info["listing_page_category"] = category_name
                ...
                for item in details_section:
                    ...
                    if detail_title=='Lokalizacja':
                        ...
                        if postcode_info:
                            single_listing_info["postcode"] = postcode_info["content"].strip()
        ...
        columns_order += [item for item in list(listing_details_translations.values()) if item not in columns_order] + ['url']
